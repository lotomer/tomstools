...
public class CharsetCheck {
    ...
    public static String getCharsetName(String content) {
        CharsetCheck checker;
        int result = OTHER;
        checker = new CharsetCheck();
        if (content.startsWith("http://") == true) {
            try {
                result = checker.detectEncoding(new URL(content));
            } catch (Exception e) {
                System.err.println("Bad URL " + e.toString());
            }
        } else {
            // result = sinodetector.detectEncoding(new File(argc[0]));
            result = checker.detectEncoding(content.getBytes());
        }
