...
public class DOMContentUtils {
  ...
  private Set<String> textNodeNames = new HashSet<String>();
  private int contentSizeMin = 1;
  ...
  public boolean getText(StringBuffer sb, Node node,
    ...
    if (getTextHelper(sb, node, abortOnNestedAnchors, 0,contentSizeMin)) {
      return true;
    }
    return false;
  }
  ...
  private boolean getTextHelper(StringBuffer sb, Node node,
      boolean abortOnNestedAnchors, int anchorDepth,int minContentSize) {
    boolean abort = false;
    NodeWalker walker = new NodeWalker(node);

    // 备份文本
    StringBuilder textBack = null;
    ...
    while (walker.hasNext()) {
      ...
      if (nodeType == Node.TEXT_NODE) {
        ...
        if (text.length() > 0) {
            if (null == textNew){
                textNew = new StringBuilder();
            }
            // 如果是段落
            if("p".equalsIgnoreCase(nodeName)
               || (textNodeNames.contains(nodeName) && null != currentNode.getParentNode() && null != currentNode.getParentNode().getParentNode() && "p".equalsIgnoreCase(currentNode.getParentNode().getParentNode().getNodeName()))){
                if (isNewSection) {
                    // 是新段落，则初始化
                    isNewSection = false;
                    //if (null == textBack ||  textBack.length() < textNew.length()){
                    //    textBack = new StringBuilder(textNew.toString());
                        textNew = new StringBuilder(text);
                    //}
                }else{
                    // 不是新段落，则直接追加文本
                    textNew.append(' ').append(text);
                }
            }else{
                // 新段落，先保存原数据，再保存当前数据
                isNewSection = true;
                ...
                if (null == textBack || textBack.length() < text.length()){
                    textBack = new StringBuilder(text);
                }
            }
        }
      }
    }
    if (null != textBack && null != textNew &&  textBack.length() < textNew.length()){
        if (isValid(textNew.toString(),minContentSize)){
            sb.append(textNew.toString());
        }
    }else if (null != textBack && textBack.length() > 0){
        if (isValid(textBack.toString(),minContentSize)){
            sb.append(textBack.toString());
        }
    }
    return abort;
  }

  private static final Pattern PATTERN_RIGHT_INFO = Pattern.compile("all\\s+rights\\s+reserved", Pattern.CASE_INSENSITIVE);
  /* 判断是否是有效文本 */
  private boolean isValid(String msg,int minSize) {
    if(null == msg || msg.length() < minSize){
        return false;
    }
    return !PATTERN_RIGHT_INFO.matcher(msg).find();
}
    ...
    while (walker.hasNext()) {
      ...
      if (nodeType == Node.ELEMENT_NODE) {
        if ("title".equalsIgnoreCase(nodeName)) {
          ...
          getTextHelper(sb, currentNode, false, 0,1);
          return true;
        }
      }
    }
...
public static void main(String[] args) throws MalformedURLException {
    String url = "http://blog.csdn.net/hainan16/article/details/6667483";
    URLUtil.getDomainName(url);
    System.out.println(URLUtil.getDomainName(url));
    System.out.println(URLUtil.getHost(url));
    System.out.println(URLUtil.getDomainSuffix(url));
    //System.out.println(isValid("© 2004-2015 www.autohome.com.cn All Rights Reserved. 汽车之家 版权所有"));
}
}
