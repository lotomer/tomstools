...
import org.tomstools.html.request.RequestInfo;
...
public class HTMLFetcher {
     ...
     * @param regexpFilterInclude   内容过滤器。需要包含哪些内容的正则表达式，包含的内容必须以分组的形式出现，并且只支持一组，如&lt;div&gt;(.*?)&lt;/div&gt;。
     *                              可为null，表示包含全部
     ...
     * @return 页面内容
     */
    public String fetchHTMLContent(String htmlUrl, String regexpFilterInclude, String regexpFilterExclude) {
        String responseText = "";
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                if (entity != null) {
                    ...
                    responseText = includeContent(new String(contents,charset),regexpFilterInclude);
                    responseText = excludeContent(responseText,regexpFilterExclude);
    ...
    String excludeContent(String content, String regexpFilterExclude) {
        ...
        if (!Utils.isEmpty(regexpFilterExclude)){
            Pattern pattern = Pattern.compile(regexpFilterExclude,
