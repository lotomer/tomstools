...
<script type="text/javascript">
	...
	$(function() {
		// 绑定事件
		query();
		initCombobox("selLang", "crawl/lang.do", function(record) {
			if (record != undefined && record.id != undefined)
				$('#LANG_ID').val(record.id);
		}, true);
		initCombobox("selCountry", "crawl/country.do", function(record) {
			if (record != undefined && record.id != undefined)
				$('#COUNTRY_CODE').val(record.id);
		}, true);
		initCombobox("selSiteType", "crawl/selectSiteType.do",
				function(record) {
					if (record != undefined && record.id != undefined)
						$('#SITE_TYPE_ID').val(record.id);
				}, true);
	...
	function selectCombobox(data) {
		if (!data) {
			$('#selLang').combobox("select", "*");
			$('#selCountry').combobox("select", "*");
			$('#selSiteType').combobox("select", "*");
		} else {
			$('#selLang').combobox("select", data.LANG_ID);
			$('#selCountry').combobox("select", data.COUNTRY_CODE);
			$('#selSiteType').combobox("select", data.SITE_TYPE_ID);
		}
	}
	var url, siteData;
	function add() {
		...
		$('#fm').form('clear');
		url = 'setting/site/add.do?key=' + key;
	}
	function edit() {
		...
		if (row) {
			...
			$('#fm').form('load', row);
			url = 'setting/site/save.do?key=' + key + '&id=' + row.SITE_ID;
			selectCombobox(row);
		}
	}
	function save() {
