...
public interface UserMapper {
     ...
     * @since 1.0
     */
    // @Select("SELECT m.MENU_ID menuId,m.MENU_NAME menuName,m.PAGE_ID pageId,m.PARENT_ID parentId,m.IS_SHOW isShow,m.ORDER_NUM orderNum,m.ICON_CLASS iconClass FROM T_M_MENUS m ,T_PRI_ROLE_MENUS rm, T_REL_ROLE_USER ru WHERE m.MENU_ID=rm.MENU_ID AND rm.ROLE_ID=ru.ROLE_ID AND ru.USER_ID=#{userId} AND  m.IS_VALID='1';")
    ...
    //@Select("SELECT rp.PAGE_ID parentId,p.PAGE_ID pageId,p.PAGE_NAME pageName,p.CONTENT_URL contentUrl,p.PARAMS,p.ICON_CLASS iconClass,p.AUTO_FRESH_TIME autoFreshTime,rp.ORDER_NUM orderNum,rp.WIDTH,rp.HEIGHT FROM T_REL_PAGES rp LEFT JOIN T_M_PAGES p ON rp.SUB_PAGE_ID=p.PAGE_ID LEFT JOIN T_PRI_ROLE_PAGES prp ON p.PAGE_ID=prp.PAGE_ID LEFT JOIN T_REL_ROLE_USER ru ON prp.ROLE_ID=ru.ROLE_ID WHERE rp.PAGE_ID=#{pageId} AND ru.USER_ID=#{userId} AND rp.IS_VALID=1 AND p.IS_VALID=1 ")
